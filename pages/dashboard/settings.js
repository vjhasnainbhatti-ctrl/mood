...
export default function SettingsPage() {
  ...
  return (
    <DashboardLayout>
      <Head><title>Settings - Moodify</title></Head>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
          <GlassCard className="p-6">
            ...
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => setDarkMode(!darkMode)} className={`relative w-14 h-7 rounded-full transition-colors ${darkMode ? 'bg-indigo-500' : 'bg-gray-300'}`}>
                <motion.div className="absolute top-1 w-5 h-5 rounded-full bg-white flex items-center justify-center" animate={{ left: darkMode ? '32px' : '4px' }} transition={{ type: 'spring', stiffness: 500, damping: 30 }}>
